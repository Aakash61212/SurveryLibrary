...
public class CheckBoxAnimated extends View implements Checkable {
    ...
    public CheckBoxAnimated(Context context) {
        this(context, null);
    }

    public CheckBoxAnimated(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CheckBoxAnimated(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public CheckBoxAnimated(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(AttributeSet attrs) {
        ...
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.CheckBoxAnimated);
        int tickColor = ta.getColor(R.styleable.CheckBoxAnimated_color_tick, COLOR_TICK);
        mAnimDuration = ta.getInt(R.styleable.CheckBoxAnimated_duration, DEF_ANIM_DURATION);
        mFloorColor = ta.getColor(R.styleable.CheckBoxAnimated_color_unchecked_stroke, COLOR_FLOOR_UNCHECKED);
        mCheckedColor = ta.getColor(R.styleable.CheckBoxAnimated_color_checked, COLOR_CHECKED);
        mUnCheckedColor = ta.getColor(R.styleable.CheckBoxAnimated_color_unchecked, COLOR_UNCHECKED);
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.CheckBoxAnimated_stroke_width, CompatUtils.dp2px(getContext(), 0));
        ta.recycle();
    ...
    public void setChecked(boolean checked) {
        ...
        invalidate();
        if (mListener != null) {
            mListener.onCheckedChanged(CheckBoxAnimated.this, mChecked);
        }
    }
    ...
    public interface OnCheckedChangeListener {
        void onCheckedChanged(CheckBoxAnimated checkBox, boolean isChecked);
    }
}
